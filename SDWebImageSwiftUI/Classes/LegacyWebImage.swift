...
public struct LegacyWebImage: View {
    ...
    public var body: some View {
        ...
        ZStack {
            ...
            if imageManager.image != nil && imageModel.url == imageManager.currentURL {
                if isAnimating && !imageManager.isIncremental && imageModel.url != nil {
                    setupPlayer()
                } else {
                    displayImage()
                }
...
extension LegacyWebImage {
    func configure(_ block: @escaping (Image) -> Image) -> LegacyWebImage {
        var result = self
    ...
    public func renderingMode(_ renderingMode: Image.TemplateRenderingMode?) -> LegacyWebImage {
        configure { $0.renderingMode(renderingMode) }
    }

    /// Configurate this view's image interpolation quality
    /// - Parameter interpolation: The interpolation quality
    public func interpolation(_ interpolation: Image.Interpolation) -> LegacyWebImage {
        configure { $0.interpolation(interpolation) }
    }

    /// Configurate this view's image antialiasing
    /// - Parameter isAntialiased: Whether or not to allow antialiasing
    public func antialiased(_ isAntialiased: Bool) -> LegacyWebImage {
...
extension LegacyWebImage {
    ...
    public func onFailure(perform action: ((Error) -> Void)? = nil) -> LegacyWebImage {
    ...
    public func onSuccess(perform action: ((PlatformImage, Data?, SDImageCacheType) -> Void)? = nil) -> LegacyWebImage {
    ...
    /// - Returns: A view that triggers `action` when this image load successes.
    public func onProgress(perform action: ((Int, Int) -> Void)? = nil) -> LegacyWebImage {
        self.imageHandler.progressBlock = action
        return self
    }
}

// LegacyWebImage Modifier
@available(iOS 13.0, OSX 10.15, tvOS 13.0, watchOS 6.0, *)
extension LegacyWebImage {
    ...
    public func placeholder<T>(@ViewBuilder content: () -> T) -> LegacyWebImage where T: View {
        var result = self
    ...
    /// - note: This placeholder image will apply the same size and resizable from LegacyWebImage for convenience. If you don't want this, use the ViewBuilder one above instead
    /// - Parameter image: A Image view that describes the placeholder.
    public func placeholder(_ image: Image) -> LegacyWebImage {
        placeholder {
    ...
    public func retryOnAppear(_ flag: Bool) -> LegacyWebImage {
    ...
    public func cancelOnDisappear(_ flag: Bool) -> LegacyWebImage {
...
extension LegacyWebImage {
    ...
    public func indicator<T>(@ViewBuilder content: @escaping (_ isAnimating: Binding<Bool>, _ progress: Binding<Double>) -> T) -> some View where T: View {
        indicator(Indicator(content: content))
    }
}
...
extension LegacyWebImage {
    ...
    public func customLoopCount(_ loopCount: UInt?) -> LegacyWebImage {
    ...
    public func maxBufferSize(_ bufferSize: UInt?) -> LegacyWebImage {
    ...
    public func runLoopMode(_ runLoopMode: RunLoop.Mode) -> LegacyWebImage {
    ...
    public func pausable(_ pausable: Bool) -> LegacyWebImage {
    ...
    public func purgeable(_ purgeable: Bool) -> LegacyWebImage {
    ...
    public func playbackRate(_ playbackRate: Double) -> LegacyWebImage {
    ...
    public func playbackMode(_ playbackMode: SDAnimatedImagePlaybackMode) -> LegacyWebImage {
...
#if DEBUG
@available(iOS 13.0, OSX 10.15, tvOS 13.0, watchOS 6.0, *)
struct LegacyWebImage_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            LegacyWebImage(url: URL(string: "https://raw.githubusercontent.com/SDWebImage/SDWebImage/master/SDWebImage_logo.png"))
                .resizable()
